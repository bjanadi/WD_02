...
class IViewMarks extends Component {
    ...
    componentDidMount() {
        ...
        fetch('http://localhost:3000/api/marks/get-marks')
            .then(res => {
                return res.json();
            })
            .then(json =>{
                    ...
                    <div>
                        <ul className="nav nav-tabs nav-justified">
                            <li className="nav-item">
                                <NavLink to="/imarks" className="nav-link" exact activeStyle={{color:'black'}}>Add Assignment Marks</NavLink>
                            </li>
                            <li className="nav-item">
                    ...
                    <div className='row'>
                        <div className='card' style={{margin: 'auto'}}>
                            <div className='card-header'>
                                <h1 className='modal-header'>MARKS OF ASSIGNMENT</h1>
                            </div>
                            <div className='card-body'>
                                <table className="table table-striped" style={{ marginTop: 20}}>
                                    ...
                                    <tbody>
                                    {this.state.data.map(assignmentMarks =>
                                        <tr key={assignmentMarks._id}>
                                            <td>{assignmentMarks.studentID}</td>
                                            <td>{assignmentMarks.marks}</td>
                                        </tr>
                                    )}
                                    </tbody>
                                </table>
